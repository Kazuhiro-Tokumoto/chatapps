...
async function processDecryption(msg: any) {
    if (!aesKeyhash) return; // 鍵がまだないなら何もしない
    ...
    try {
        ...
        if (["image", "file", "audio"].includes(msg.subType)) {
            ...
            if (!msg.mimeType && msg.fileName) {
                ...
                else if (fname.endsWith(".mp4")) mime = "video/mp4";
            }
...
btnroom.addEventListener("click", async () => {
    ...
    const targetProfile = await PublicKeyFetch(inputVal, supabase);
        ...
        try {
            ...
            if (!pin) throw new Error("PINコードが見つかりません。再ログインしてください。");
            ...
            const theirPublicKey = await window.crypto.subtle.importKey(
                "raw",
                theirRawKey.buffer as ArrayBuffer,
                { name: "X25519" },
                true,
                []
            );
...
wss.onmessage = async (event) => {
    ...
    // ------------------------------------------------
        ...
        if (data.uuid === storedUuid) return;
    ...
    // ------------------------------------------------
    else if (data.type === "join-ack") addSystemMsg("参加しました");
    ...
    try {
        ...
        // --- メディア（画像・音声・ファイル）の場合 ---
            ...
            addMediaBubble(
                url, 
                msg.name || "Unknown", 
                msg.originalName || msg.fileName || "file", 
                isMe, 
                msg.subType // ここで subType を渡すことで addMediaBubble 内で分岐される
            );
        }
